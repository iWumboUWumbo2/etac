...
public class CFGGraph<T> {
    ...
    public CFGGraph(ArrayList<T> stmts) {
        ...
        for (int i = 0; i < stmts.size(); i++) {
            ...
            if (i != 0
                    && !(nodes.get(i-1).stmt instanceof IRReturn)
                    && !(nodes.get(i-1).stmt instanceof IRJump)
                    && !(nodes.get(i-1).stmt instanceof ASMRet)
                    && !(nodes.get(i-1).stmt.equals(outOfBounds))
                    && !(nodes.get(i-1).stmt instanceof ASMJumpAlways)
                    && !(nodes.get(i-1).stmt.equals(asmOut))
            ) {
                cfgnode.addPredecessor(nodes.get(i - 1));
            }

            if (i != stmts.size() - 1
                    && !(nodes.get(i).stmt instanceof IRReturn)
                    && !(nodes.get(i).stmt instanceof ASMRet)
                    && !(nodes.get(i).stmt instanceof IRJump)
                    && !(nodes.get(i).stmt.equals(outOfBounds))
                    && !(nodes.get(i).stmt instanceof ASMJumpAlways)
                    &&!(nodes.get(i).stmt.equals(asmOut))) {
